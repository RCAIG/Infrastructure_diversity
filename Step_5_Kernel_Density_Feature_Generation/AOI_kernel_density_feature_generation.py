...
input_shp_dir = r"Your AOI point feature path"
...
for i, shp_file in enumerate(shp_files):
    try:
        ...
        # Get the value of the "aoi_re" field in the shapefile
        aoi_re_values = ['residential','commercial','education','medical','public']
        ...
        # Iterate over each "aoi_re" value
        for aoi_type in aoi_re_values:

            # Use the "Select Feature" tool to select the feature corresponding to the current "aoi_re" value
            selected_features = os.path.join(workspace, f"{aoi_type}_selected.shp")
            a = arcpy.management.SelectLayerByAttribute(shp_path, "NEW_SELECTION", f"bui_big = '{aoi_type}'")
            ...
            output_tif_500 = os.path.join(workspace, f"{aoi_type[:3]}_500.tif")
            ...
            output_tif_250 = os.path.join(workspace, f"{aoi_type[:3]}_250.tif")
