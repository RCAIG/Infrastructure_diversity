...
input_shp_dir = r"Your landuse point feature path"
...
for i, shp_file in enumerate(shp_files):
    try:
        ...
        # Get the value of the "landuse_re" field in the shapefile
        landuse_re_values = ['residential','commercial','education','medical','public']
        ...
        # Iterate over each "landuse_re" value
        for landuse_type in landuse_re_values:

            # Use the "Select Feature" tool to select the feature corresponding to the current "landuse_re" value
            selected_features = os.path.join(workspace, f"{landuse_type}_selected.shp")
            a = arcpy.management.SelectLayerByAttribute(shp_path, "NEW_SELECTION", f"bui_big = '{landuse_type}'")
            ...
            output_tif_500 = os.path.join(workspace, f"{landuse_type[:3]}_500.tif")
            ...
            output_tif_250 = os.path.join(workspace, f"{landuse_type[:3]}_250.tif")
