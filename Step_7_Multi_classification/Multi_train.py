...
# Define the base path for the input directory and range of years
base_directory = r'your kernel density path'
...
for year in years:
    ...
    for filename in os.listdir(input_directory):
        if filename.endswith('.csv'):
            ...
            # Filter data, keeping rows where “bui_lab” is not “other” or “residential”
            data = data[(data['bui_lab'] != 'other') & (data['bui_lab'] != 'residential')]
    ...
    predictor = TabularPredictor(label=label, eval_metric='balanced_accuracy').fit(train_data, tuning_data=val_data)
    ...
    eval_file_path = os.path.join(r'your result path', f'evaluate_Multi_{year}.csv')
    ...
    model_folder = r'your result path'  # Create a subdirectory to save the model for that year
    os.makedirs(model_folder, exist_ok=True)  # Create the directory, do not raise an error if it already exists

    model_name = os.path.join(model_folder, f"Multi_model_{year}")  # Custom full path for the model
